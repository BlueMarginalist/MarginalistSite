...
export default async function ProjectDetailPage({
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-16 sm:py-24">
      <Link
        href={sectionConfig.projects.path}
      ...
      <header className="mb-10">
        ...
        {(frontmatter.links?.demo ||
          ...
          <div className="flex flex-wrap gap-5 mt-6 pt-6 border-t border-zinc-200 dark:border-zinc-800">
            ...
            {frontmatter.links.external && (
              <a
                href={frontmatter.links.external}
                target="_blank"
                rel="noopener noreferrer"
                className="text-sm font-medium text-blue-700 dark:text-blue-400 hover:underline"
              >
      ...
      {/* Embedded demo — shown when `iframe` is set in frontmatter */}
      {frontmatter.iframe && (
        <div className="mb-10">
          <IframeEmbed src={frontmatter.iframe} title={frontmatter.title} />
        </div>
      )}
